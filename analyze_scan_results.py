"""Strict diagnostics for completed N=4/N=6/N=8 period-scan CSV files.

This script intentionally performs post-processing only. It reads the merged
N=4/N=6/N=8 period-scan CSV files already present in outputs/ and writes Chinese data
...
def build_paths(array_size: int) -> dict[str, Path]:
    prefix = f"N{array_size}"
    return {
        "summary_csv": OUTPUT_DIR / f"{prefix}_period_scan_summary_0p90_2p00.csv",
        "point_csv": OUTPUT_DIR / f"{prefix}_period_scan_point_probes_0p90_2p00.csv",
        "line_csv": OUTPUT_DIR / f"{prefix}_period_scan_line_probes_0p90_2p00.csv",
        "report_md": OUTPUT_DIR / f"{prefix}_analysis_report.md",
        "key_metrics_csv": OUTPUT_DIR / f"{prefix}_key_metrics.csv",
        "band_peak_csv": OUTPUT_DIR / f"{prefix}_band_peak_summary.csv",
        "center_probe_csv": OUTPUT_DIR / f"{prefix}_center_probe_summary.csv",
        "line_profile_csv": OUTPUT_DIR / f"{prefix}_line_profile_summary_selected_periods.csv",
        "plot_recommendations_csv": OUTPUT_DIR / f"{prefix}_plot_recommendations.csv",
    }
...
def require_input_files(paths) -> None:
    missing = [path for path in (paths["summary_csv"], paths["point_csv"], paths["line_csv"]) if not path.exists()]
    if missing:
        missing_text = "\n".join(f"- {path}" for path in missing)
        raise FileNotFoundError(
            "缺少指定阵列规模的已合并周期扫描 CSV，无法生成数据诊断报告。"
            "请先把以下原始 CSV 放入 outputs/，本脚本不会生成或修改这些原始文件：\n"
            f"{missing_text}"
        )


def read_inputs(paths: dict[str, Path]) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    require_input_files(paths)
    summary_df = pd.read_csv(paths["summary_csv"])
    point_df = pd.read_csv(paths["point_csv"])
    line_df = pd.read_csv(paths["line_csv"])
...
def validate_summary(summary_df: pd.DataFrame) -> list[ValidationResult]:
    ...
    results.append(ValidationResult("summary exists", "PASS", str(CURRENT_PATHS["summary_csv"])))
...
def validate_points(point_df: pd.DataFrame) -> list[ValidationResult]:
    results: list[ValidationResult] = [ValidationResult("point exists", "PASS", str(CURRENT_PATHS["point_csv"]))]
...
def validate_lines(line_df: pd.DataFrame) -> list[ValidationResult]:
    results: list[ValidationResult] = [ValidationResult("line exists", "PASS", str(CURRENT_PATHS["line_csv"]))]
...
def plot_recommendations(
    ...
    rows = [
        {
            "figure_name": f"N{array_size}_center_five_points_0p90_2p00",
            "recommended": "yes",
            "reason": "用于检验 P0-P4 的峰值周期和幅值是否一致，避免把中心响应过度概括为单点行为。",
            "results_position": "Results 中 N={array_size} 中心响应讨论的第一组辅助图或补充图。",
        ...
        {
            "figure_name": f"N{array_size}_center_summary_0p90_2p00",
            "recommended": "yes",
        ...
        {
            "figure_name": f"N{array_size}_front_rear_S_0p90_2p00",
            "recommended": "yes",
        ...
        {
            "figure_name": f"N{array_size}_line_profiles_selected_periods",
            "recommended": "yes",
        ...
        {
            "figure_name": f"N{array_size}_key_period_map_or_table",
        ...
        {
            "figure_name": f"N{array_size}_mesh_convergence_table",
            "recommended": "yes",
            "reason": "用于方法或验证部分说明正式 N={array_size} 扫描网格选择的可靠性。",
            ...
            "does_not_support": "不能由本 N={array_size} 响应 CSV 单独证明；需要独立网格收敛数据。",
        },
    ]
...
def write_report(
    ...
    text = f"""# N={array_size} 周期扫描结果数据诊断报告

本报告由 `analyze_scan_results.py` 生成，仅读取既有 CSV，不运行 `solver.solve(...)`，不修改原始 outputs CSV，也不生成最终论文图。所有中心响应、前后场响应和测线空间分布判断均基于 `total_abs`；`diffracted_abs` 仅可作为散射贡献参考。`S_rear_front = rear_abs / front_abs` 在本报告中只称为 transmission-like indicator，不称为严格 transmission coefficient 或 Kt。
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Analyze merged period-scan CSV outputs for ring arrays.")
...
def main() -> None:
    ...
    paths = build_paths(array_size)
    CURRENT_PATHS.update(paths)

    summary_df, point_df, line_df = read_inputs(paths)
    validation_results = []
    ...
    for df in (summary_df, point_df, line_df):
        if "array_size" in df.columns and not (pd.to_numeric(df["array_size"], errors="coerce") == array_size).all():
            raise ValueError(f"array_size column contains values not equal to --array-size {array_size}.")
        if "mesh_level" in df.columns and not (df["mesh_level"].astype(str) == "medium").all():
            raise ValueError("mesh_level column must be medium for all rows.")
    ...
    plot_recs = plot_recommendations(array_size, center_peak_match, rear_s_peak_match, center_uniform)

    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    key_metrics.to_csv(paths["key_metrics_csv"], index=False)
    band_summary.to_csv(paths["band_peak_csv"], index=False)
    center_summary.to_csv(paths["center_probe_csv"], index=False)
    line_summary.to_csv(paths["line_profile_csv"], index=False)
    plot_recs.to_csv(paths["plot_recommendations_csv"], index=False)
    write_report(
        array_size,
        paths["report_md"],
        validation_results,
