        ...
        rows_out.append(
            {
                ...
                "boundary_note": "near lower boundary: " + ", ".join(boundary_terms)
                if boundary_terms
                else "none",
            }
        )
...
def plot_figures(series_by_n: dict[int, SeriesData]) -> list[str]:
    ...
    try:
        ...
        return ["No figures were generated because matplotlib is not installed."]
...
def describe_table_rows(table_rows: list[dict[str, str]]) -> str:
    if not table_rows:
        return (
            ...
            "data-derived extrema for Table Z could not be extracted. The CSV file was "
            "written with headers only to avoid fabricated values."
        )
    parts = []
    for row in table_rows:
        parts.append(
            f"For N={row['N']}, A_front ranges from {row['A_front_min']} at T={row['T_A_front_min_s']} s "
            f"to {row['A_front_max']} at T={row['T_A_front_max_s']} s; A_rear ranges from "
            f"{row['A_rear_min']} at T={row['T_A_rear_min_s']} s to {row['A_rear_max']} at "
            f"T={row['T_A_rear_max_s']} s; S_rear/front ranges from {row['S_rear_front_min']} "
            f"at T={row['T_S_rear_front_min_s']} s to {row['S_rear_front_max']} at "
            f"T={row['T_S_rear_front_max_s']} s. Boundary note: {row['boundary_note']}."
        )
    return "\n\n".join(parts)
...
Table Z lists the extrema of A_front, A_rear, and S_rear/front together with their corresponding periods. {data_text} Near-boundary extrema should be interpreted cautiously, especially when an extremum occurs at T = 0.90 s or within the T = 0.90-0.93 s inspection interval. These extrema are not interpreted here as confirmed resonance or as evidence of a modal mechanism.
